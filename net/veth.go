...
func CreateAndAttachVeth(name, peerName, bridgeName string, mtu int, forceTXOn bool, init func(peer netlink.Link) error) (*netlink.Veth, error) {
	...
	case Bridge, BridgedFastdp:
		...
		if bridgeType == Bridge && !forceTXOn {
...
func AttachContainer(ns netns.NsHandle, id, ifName, bridgeName string, mtu int, withMulticastRoute bool, cidrs []*net.IPNet, forceTXOn bool) error {
	if !interfaceExistsInNamespace(ns, ifName) {
		...
		_, err := CreateAndAttachVeth(name, peerName, bridgeName, mtu, forceTXOn, func(veth netlink.Link) error {
